const caregiverTerms = `
TERMS OF SERVICE AND PRIVACY POLICY FOR CAREGIVERS

Effective Date: January 1, 2024
Version: 1.0

=== TERMS OF SERVICE ===

1. Definitions

Platform – The CareConnect platform, including web and mobile applications.

Caregiver – A verified healthcare professional offering services through the platform.

2. Acceptance of Terms

By registering and using the platform, caregivers agree to be bound by these terms and privacy policy.

3. Eligibility

Caregivers Must:
• Hold valid qualifications and licenses.
• Provide truthful registration details.
• Submit to verification and background checks.

4. Roles and Responsibilities

Caregivers must:
• Deliver safe and professional care.
• Maintain patient confidentiality.
• Submit accurate care session reports.
• Properly indicate patient status (Stable, Improving, Critical, Cured, Deceased).
• Immediately report critical incidents or death.

5. Verification & Credential Management

The system reserves the right to verify or re-verify qualifications at any time.

6. Authorization for Data Verification

By registering as a caregiver, you authorize us to:
• Contact and verify information with previous clients or employers regarding your professional performance and conduct.
• Contact training institutions, licensing bodies, and educational facilities to verify your qualifications, certifications, and professional standing.
• Conduct background checks and reference verification as deemed necessary for patient safety and platform integrity.
• Share relevant verification results with patients and healthcare authorities as required for informed decision-making.

This authorization remains valid throughout your registration with the platform and for a reasonable period thereafter as required for ongoing compliance and safety verification.

7. Confidentiality & Data Protection

All patient and system data must be kept confidential and handled according to applicable laws.

8. Data Protection & Privacy (Malawi Data Privacy Protection Act Compliance)

By registering with our platform, you acknowledge and consent to:
• Collection, processing, and storage of your personal and professional data as necessary for platform operation and healthcare service delivery.
• Background checks and credential verification processes.
• Data sharing with patients, healthcare authorities, and regulatory bodies as required.
• Data retention for legal, professional, and regulatory compliance requirements.
• Your rights under the Malawi Data Privacy Protection Act, including the right to access, correct, or request deletion of your personal data.
• Automated email notifications regarding your account, appointments, compliance matters, and data protection policies.
• Cross-border data transfers where necessary for service provision, with appropriate safeguards.

You have the right to:
• Access your personal data held by the platform.
• Request correction of inaccurate personal data.
• Request deletion of your personal data (subject to legal, professional, and regulatory record retention requirements).
• Withdraw consent for non-essential data processing.
• Lodge complaints with the Malawi Data Protection Authority.

As a healthcare professional, you acknowledge that certain data retention and sharing requirements may override deletion requests due to professional and legal obligations.

9. Payments & Billing Rules

Payments must be processed only through the platform.
Unauthorized off-platform transactions are prohibited.

10. Prohibited Conduct

Users must not:
• Submit false information
• Abuse privileges
• Discriminate or harm users
• Access records without authorization

11. Disciplinary Actions

Possible actions include:
• Warnings
• Suspension
• Termination
• Legal or professional referral

12. Limitation of Liability

The platform acts as a facilitator and does not replace professional medical responsibility.

13. Termination of Access

Access may be revoked for breaches of these terms without prior notice.

14. Account Deletion

You may delete your account at any time through your profile settings. Account deletion will:
• Remove your personal information from active systems
• Cancel all future appointments
• Retain professional records as required by law
• Process within 30 days of request

Note: Some data may be retained for legal, professional, and regulatory compliance.

15. Dispute Resolution

Disputes will use mediation and arbitration before court action.

16. Amendments

The platform may revise these terms and notify users of significant changes via email and platform notifications.

17. Governing Law

This agreement is governed by the laws of Malawi, including the Malawi Data Privacy Protection Act and applicable healthcare professional regulations.

=== PRIVACY POLICY ===

18. Information We Collect

We collect the following types of information:
• Personal identification information (name, email, phone, ID number)
• Professional credentials (licenses, qualifications, experience)
• Background check results and verification documents
• Location data (region, district, traditional authority, village)
• Usage data (platform interactions, appointment history, patient interactions)
• Communication data (messages, support requests)
• Financial information (earnings, payment details)

19. How We Use Your Information

We use your information to:
• Verify your professional credentials and background
• Match you with appropriate patients
• Process payments and earnings
• Monitor service quality and compliance
• Send important notifications and updates
• Comply with healthcare regulations and legal requirements
• Report to regulatory bodies as required
• Improve our services and platform functionality

20. Information Sharing

We may share your information with:
• Patients who book your services
• Healthcare regulatory authorities for compliance
• Background check and verification services
• Payment processors for transaction processing
• Legal authorities when required by law
• Professional licensing bodies as required
• Service providers who assist in platform operations (under strict confidentiality agreements)

We never sell your personal information to third parties.

21. Professional Obligations

As a healthcare professional, you acknowledge:
• Certain data must be retained for professional and legal compliance
• Patient data access is logged and monitored
• Regulatory reporting may require data disclosure
• Professional misconduct may result in data sharing with authorities

22. Data Security

We implement appropriate technical and organizational measures to protect your data:
• Encryption of sensitive data in transit and at rest
• Regular security audits and updates
• Access controls and authentication systems
• Staff training on data protection practices
• Compliance with healthcare data security standards

23. Data Retention

We retain your data for as long as necessary to:
• Provide ongoing platform services
• Comply with healthcare professional regulations
• Meet legal and regulatory requirements
• Resolve disputes and enforce agreements

Professional records may be retained for extended periods as required by healthcare regulations.

24. Your Privacy Rights

Under the Malawi Data Privacy Protection Act, you have the right to:
• Access your personal data
• Correct inaccurate information
• Request deletion of your data (subject to professional and legal requirements)
• Object to certain processing activities
• Data portability
• Lodge complaints with supervisory authorities

Note: Some rights may be limited due to professional and regulatory obligations.

25. Acceptance Statement

By using the system, you confirm that you have read, understood, and agreed to these terms of service and privacy policy, including your data protection rights under Malawi law and your professional obligations as a healthcare provider.
`;

module.exports = caregiverTerms;